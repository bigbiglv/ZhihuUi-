...
import { Heart, ThumbsUp, ChevronDown, Loader2, Pin, Flame } from 'lucide-vue-next'
...
import type { RootComment } from '@/components/ArticleCard/types'
...
async function loadMoreChildComments(comment: RootComment) {
  ...
  try {
    ...
    if (res && Array.isArray(res.data)) {
      ...
      comment.childComments.push(...res.data)
      comment.childCommentsNextUrl = res.paging?.next || null
      comment.isChildEnd = res.paging?.is_end ?? true
    }
  } catch (err) {
