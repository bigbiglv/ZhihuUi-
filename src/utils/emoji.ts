export const ZHIHU_EMOJI_MAP: Record<string, string> = {
  "[感谢]": "https://pic1.zhimg.com/v2-e39d5eebfef8b0ac6065ad156cb05e66.png",
  "[哇]": "https://pic1.zhimg.com/v2-70c38b608df613d862ee0140dcb26465.png",
  ...
  "[蹲]": "https://pic1.zhimg.com/v2-4779ff07dfe6b722cacfcf3c5185357d.png",
...
/**
 * 替换文本或HTML中的知乎文本表情（如 [捂脸]）为知乎原生图片标签
 */
export function replaceZhihuEmojis(html: string): string {
  if (!html) return html
  
  // 仅替换映射表中存在的表情，并转换为带有 .emotion 类的 img 标签
  return html.replace(/\[([^\]]{1,6})\]/g, (match) => {
    const url = ZHIHU_EMOJI_MAP[match]
    if (url) {
      return `<img src="${url}" alt="${match}" class="emotion ztext-emoji" style="display: inline-block; height: 18px; width: 18px; vertical-align: text-bottom; margin: 0;">`
    }
    return match
  })
}
